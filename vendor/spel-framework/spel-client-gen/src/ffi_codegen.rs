...
pub fn generate_ffi(idl: &SpelIdl) -> Result<String, String> {
    ...
    writeln!(out, "//   - `wallet_path`: path to LEE wallet directory").unwrap();
    writeln!(
        out,
                    ...
                    match &pda.seeds[0] {
                        ...
                        IdlSeed::Arg { path } => {
                            ...
                            if arg_ty == "u64" {
                                ...
                                writeln!(
                                    out,
                                    "    seed_bytes[..8].copy_from_slice(&{}.to_le_bytes());",
                                    pname
                                )
                                .unwrap();
                            } else {
                                writeln!(out, "    let seed_bytes: [u8; 32] = *{};", pname)
                                    .unwrap();
                            }
                        }
                        IdlSeed::Account { path } => {
                        ...
                        match seed {
                            ...
                            IdlSeed::Arg { path } => {
                                ...
                                if arg_ty == "u64" {
                                    ...
                                    writeln!(out, "    hasher.update(&{}.to_le_bytes());", pname)
                                        .unwrap();
                                } else {
                                    writeln!(out, "    hasher.update({} as &[u8]);", pname)
                                        .unwrap();
                                }
                            }
                            IdlSeed::Account { path } => {
                                ...
                                writeln!(
                                    out,
                                    "    // TODO: account seed '{}' — use account bytes",
                                    path
                                )
                                .unwrap();
...
pub fn generate_account_fetch_functions(idl: &SpelIdl, prefix: &str, out: &mut String) {
    ...
    for ix in &idl.instructions {
        for acc in &ix.accounts {
            ...
            // Build a type map for seed arg lookups
            let param_type_map: std::collections::HashMap<String, String> = ix
                .args
                .iter()
                .map(|a| (rust_ident(&a.name), idl_type_to_rust(&a.type_)))
                    ...
                    IdlSeed::Arg { path } => {
                        ...
                        let parse_expr = idl_type_to_json_parse(
                            &ix.args
                                ...
                                .map(|a| a.type_.clone())
                                .unwrap_or(spel_framework_core::idl::IdlType::Primitive(
                                    "String".to_string(),
                                )),
                            &format!("args[\"{path}\"]"),
                        );
