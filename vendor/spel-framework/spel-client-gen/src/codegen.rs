...
pub fn generate_client(idl: &SpelIdl) -> Result<String, String> {
    ...
    writeln!(
        out,
        "//! Auto-generated client for the {} program.",
        idl.name
    )
    .unwrap();
    writeln!(
        out,
        "//! Generated by spel-client-gen from IDL v{}.",
        idl.version
    )
    .unwrap();
    ...
    for ix in &idl.instructions {
        ...
        for acc in ix.accounts.iter().filter(|a| a.rest) {
            writeln!(
                out,
                "        account_ids.extend_from_slice(&accounts.{});",
                rust_ident(&acc.name)
            )
            .unwrap();
        }
    ...
    for helper in &pda_helpers {
        ...
        writeln!(
            out,
            "    /// Fetch and deserialize the `{}` account state.",
            helper.account_name
        )
        .unwrap();
        write!(
            out,
            "    pub async fn fetch_{}<T: BorshDeserialize>(\n        &self",
            helper.account_name
        )
        .unwrap();
        for (pname, pty) in &helper.params {
        ...
        write!(
            out,
            "        let account_id = compute_{}_pda(&self.program_id",
            helper.account_name
        )
        .unwrap();
